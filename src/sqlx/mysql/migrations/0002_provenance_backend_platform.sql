-- mediaschema — MySQL migration 0002: speaker voiceprint provenance
...
--
-- NOTE: MySQL `ALTER TABLE ... ADD COLUMN` is not idempotent (no
-- `IF NOT EXISTS`); a migration runner must apply each file exactly once.

ALTER TABLE speaker ADD COLUMN voiceprint_provenance_backend             SMALLINT;
ALTER TABLE speaker ADD COLUMN voiceprint_provenance_platform_os         VARCHAR(256);
ALTER TABLE speaker ADD COLUMN voiceprint_provenance_platform_arch       VARCHAR(256);
ALTER TABLE speaker ADD COLUMN voiceprint_provenance_platform_os_version VARCHAR(256);
